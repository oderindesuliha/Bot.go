...
func TestRegisterUnregisterListWebhooks(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/discord/webhooks/register", bytes.NewBuffer(body))
	...
	// List webhooks and check it exists
	listReq := httptest.NewRequest("GET", "/discord/webhooks", nil)
	...
	// Unregister the created webhook
	delPayload := map[string]string{"id": created.ID}
	delBody, _ := json.Marshal(delPayload)
	delReq := httptest.NewRequest("DELETE", "/discord/webhooks/unregister", bytes.NewBuffer(delBody))
	...
	handler.HandleListWebhooks(listRec2, httptest.NewRequest("GET", "/discord/webhooks", nil))
